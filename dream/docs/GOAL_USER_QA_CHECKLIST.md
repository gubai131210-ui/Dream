...
预计手测约 **15–25 分钟**（户外全表 + 室内抽样）。  
从清单「跳转」进入场景后，右上角会出现 **「回§7清单」** 与 **本项手测要点** 浮层，测完可立即返回继续勾选。  
可用 **「跳转下一项未勾」** 按进度顺序测；每项下方有绿色 **通过标准**。
...
路灯手感：白天点路灯会切夜间观灯；**关灯回白天**（或顶栏「夜间」/按 **N**）。灯塔/车站同理。

## Agent 侧（已绿，不代替本表）

- 全套 `tools/qa_*.py` GREEN  
- Agent walkthrough / MCP 证据索引见 `GOAL_INTERACT_COMPLETE` G8  
- Genre/Canon/Style 有签收；**不能**代替手感勾选  
