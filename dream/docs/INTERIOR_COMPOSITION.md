...
8. **整体感**：房间是一次构图（主锚 + 次区 + 负空间），不是“放满了”。验收见 `interior-visual-qa` **Ensemble**。  
9. **交互/人物承接**：每个使用点前留站位；主路可并排 玩家+NPC；热点提示不挡门。验收见 **Interact-ready**。
...
- 禁止店床同厅、谷仓无过道畜栏对  
- 禁止只改标题不改 `clusters`  
- 禁止「放下就算」、无主次/无负空间仍报整体合格  
- 禁止塞满到无法并排走人、无法站在柜台/床/箱前交互  
