...
@onready var btn_hill: Button = $UI/TopBar/ToHill
@onready var btn_light: Button = $UI/TopBar/ToLight
@onready var grid_overlay: Node2D = $DebugGrid


func _ready() -> void:
	var assembler := get_node_or_null("Assembler") as LakeAssembler
	if assembler:
	...
	if btn_hill:
		btn_hill.pressed.connect(func(): get_tree().change_scene_to_file(SceneRouter.HILL_FARM_PATH))
	if btn_light:
		btn_light.pressed.connect(func(): get_tree().change_scene_to_file(SceneRouter.LIGHTHOUSE_PATH))
	...
	_dik.attach_to(self, "lake", top_bar)
